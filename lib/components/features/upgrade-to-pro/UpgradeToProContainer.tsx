...
const menu: Record<string, { label: string; icon: string; component: React.FC<{ space: Space; data?: any }> }> = {
  overview: {
...
function UpgradeToProContainer({ subscriptionData }: { subscriptionData: SubscriptionItem[] }) {
  const router = useRouter();
  const me = useMe();

  const [active, setActive] = React.useState('plans');
      ...
      <div className="h-dvh flex flex-col md:flex-row">
        ...
        {space && (
          <div className="w-full flex-1 overflow-auto no-scrollbar">
            <Comp space={space} data={subscriptionData} />
          </div>
        )}
      </div>
    </>
