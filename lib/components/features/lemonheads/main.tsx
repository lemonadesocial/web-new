          ...
          {mintPrice && ethers.formatEther(mintPrice)}
          {
            mintPrice && (
              <DaimoPayButton
                ...
                toChain={Number(LEMONHEAD_CHAIN_ID)}
                toUnits={ethers.formatEther(mintPrice)}
                ...
                toCallData={encodeFunctionData({
                  ...
                  functionName: "mint",
                  args: [mintData.look as `0x${string}`, mintData.metadata, mintData.signature as `0x${string}`],
                })}
                paymentOptions={['AllPaymentApps']}
                preferredChains={[Number(LEMONHEAD_CHAIN_ID)]}
                preferredTokens={[
                  { chain: Number(LEMONHEAD_CHAIN_ID), address: ethers.ZeroAddress as `0x${string}` }
                ]}
