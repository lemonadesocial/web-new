...
export function TicketSelectItem({ ticketType, single }: { ticketType: PurchasableTicketType; single?: boolean }) {
  ...
  return (
    <div className={clsx('flex p-3 rounded-sm justify-between items-start', active ? 'border border-tertiary' : 'border border-transparent bg-tertiary/8')}>
      <div>
        <p className={clsx('font-medium', active ? 'text-primary' : 'text-secondary')}>{ticketType.title}</p>
  ...
  if (secondPrice) {
    return (
      <div className={clsx('flex items-baseline', single ? 'gap-2' : 'gap-1.5')}>
        <p className={clsx(single ? 'text-xl font-semibold' : 'text-medium font-medium', active || single ? 'text-primary' : 'text-secondary')}>
          {formatPrice(firstPrice)}
        </p>
        <p className={clsx('font-medium', single ? 'text-sm text-secondary' : 'text-medium', active || single ? 'text-primary' : 'text-secondary')}>
