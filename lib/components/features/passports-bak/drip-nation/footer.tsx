...
export function PassportFooter() {
  ...
  const steps = PASSPORT_CONFIG[state.provider].steps;
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/passport/${BackendPassportProvider.DripNation}?wallet=${address}&avatar=${encodeURIComponent(state.photo)}&username=${username}`,
      );
      ...
      <div className="hidden md:flex justify-between items-center min-h-[64px] px-4 bg-background/80 backdrop-blur-md">
        ...
        {PassportStep.intro !== state.currentStep && (
          <ul className="flex items-center justify-center flex-2 gap-1.5">
            {Object.entries(steps).map(([key, item], idx) => {
              ...
              return (
                <li key={key} className="flex items-center gap-1.5">
                  ...
                  {item.label && idx < Object.entries(steps).length - 1 && (
