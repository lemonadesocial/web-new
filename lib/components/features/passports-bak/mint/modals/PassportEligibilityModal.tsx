...
export function PassportEligibilityModal({ onContinue }: { onContinue: () => void }) {
  ...
  return (
    <ModalContent>
      <div className="flex flex-col gap-4">
        <img src={`${ASSET_PREFIX}/assets/images/lemonheads-getstarted.gif`} className="rounded-sm size-14" alt="LemonHead" />

        <div>
          <p className="text-lg">Claim Your LemonHead</p>
          <p className="text-sm text-secondary">Passports are only available to LemonHead holders. Claim yours or switch wallets.</p>
        </div>
