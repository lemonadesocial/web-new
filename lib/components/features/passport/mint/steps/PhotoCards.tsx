  ...
  return (
    <div className={`flex flex-col gap-4 rounded-md bg-card p-4 transition-all border ${isSelected ? 'border-primary' : 'border-card-border'}`}>
      {loading || !data ? (
        ...
        <>
          <img src={data.image} className={clsx("w-full h-full border-card-border rounded-sm")} />
          <div className="flex flex-col gap-1">
            <p>LemonHead #{data.tokenId}</p>
            <p className="text-sm text-tertiary">Set LemonHead as Passport Photo.</p>
          </div>
          <Button
...
export function FluffleCard() {
    ...
    return (
      <div className={`flex flex-col gap-4 rounded-md bg-card p-4 transition-all border ${isSelected ? 'border-primary' : 'border-card-border'}`}>
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-tertiary">Set Fluffle as Passport Photo.</p>
        </div>
        <Button
          variant={isSelected ? "tertiary" : "secondary"}
          className="w-full"
  ...
  return (
    <div className="flex flex-col gap-4 rounded-md bg-card p-4 transition-all border-2 border-dashed border-card-border">
      <img src={`${ASSET_PREFIX}/assets/images/fluffle.png`} className="w-full" />
      <div className="flex flex-col gap-1">
          <p>Get Fluffle</p>
          <p className="text-sm text-tertiary">You don’t have a fluffle, yet.</p>
        </div>
        <Button
          variant={isSelected ? "tertiary" : "secondary"}
          className="w-full"
