...
export function CommunitySearch({ onSuccess }: CommunitySearchProps) {
  ...
  if (selectedGroup) return (
    <div className="flex items-center gap-3 rounded-md border border-card-border bg-card px-3 py-2">
      <img src={randomCommunityImage(selectedGroup.address)} alt={selectedGroup.name} className="size-9.5 rounded-sm object-cover" />
    ...
    >
      ...
      <Menu.Content className="p-0 w-full max-h-52 overflow-y-auto">
        {groups.map((group) => (
          ...
          >
            <img src={randomCommunityImage(group.address)} alt={group.name} className="size-9.5 rounded-sm object-cover" />
