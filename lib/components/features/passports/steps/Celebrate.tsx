...
export function PassportCelebrate() {
  ...
  React.useEffect(() => {
    if (videoRef.current && !playerRef.current && state.ui?.celebrate.showBgVideo) {
        ...
        <div className="flex-1 flex flex-col items-center gap-11 pt-6 pb-11 max-sm:justify-center w-full">
          ...
          <div className="flex flex-wrap gap-5 w-full justify-center md:justify-between">
            <div className="flex items-center gap-2">
              {state.ui?.celebrate.showBgVideo && (
                <Button
              ...
              <Button
                ...
                variant="tertiary-alt"
                onClick={() =>
                  window.open(`${chainsMap[PASSPORT_CHAIN_ID]?.block_explorer_url}/tx/${state.mintState?.txHash}`)
                }
              >
                View txn.
              </Button>
