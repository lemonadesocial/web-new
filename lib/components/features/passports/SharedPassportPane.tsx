...
export function SharedPassportPane({
  provider,
  passportTitle,
  tokenId,
}: {
  ...
  const shareUrl = `${window.location.origin}/api/og/passport/${provider}?tokenId=${tokenId}`;
  const shareText = `Just claimed my ${passportTitle} Passport Fully onchain, totally me. Yours is waiting—go mint it now → `;
