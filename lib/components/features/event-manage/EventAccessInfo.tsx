  ...
  return (
    <div className="rounded-md border border-card-border bg-card p-3 grid grid-cols-2 gap-5">
      ...
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-secondary">{`${window.location.host}/e/${event.shortid}`}</p>
        <div className="flex items-center gap-4">
          ...
          />
          <i
            className="icon-copy text-tertiary size-5 cursor-pointer"
            onClick={handleCopy}
          />
          <i
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-4 w-full text-nowrap">
          <div className="border rounded-sm size-12 min-w-12 flex flex-col overflow-hidden">
            ...
            <p className="text-center">{format(convertFromUtcToTimezone(event.start, event.timezone as string), 'dd')}</p>
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            <Button variant="tertiary" size="sm" onClick={() => drawer.open(EditEventDrawer, { props: { event }, dismissible: false })} className="flex-1">
              Edit Event
            </Button>
            <FileInput onChange={handleFileChange} multiple={false} accept="image/*" className="flex-1">
              {(open) => (
                <Button variant="tertiary-alt" size="sm"  className="w-full" onClick={open} loading={uploading} disabled={uploading}>
                  Change Photo
                </Button>
              )}
            </FileInput>
