...
export function AddEmailsModal({ ticketType }: { ticketType: EventTicketType }) {
    ...
    try {
      await createTickets({
        variables: {
          ...
          ticketAssignments: addedEmails.map(({ email }) => ({
            email,
            count: 1,
          })),
        },
    ...
    <Card.Root className="w-sm max-w-full md:w-[448px] *:bg-overlay-primary border-none">
      ...
      <Card.Content className="flex flex-col p-0">
        <div className="flex flex-col gap-2 px-4">
          <p className="text-lg">Add Emails</p>
          <p className="text-sm">
            Only the email addresses you add below will be able to purchase this ticket. Please note: anyone can still
            view the ticket.
          </p>
        </div>
            ...
            {addedEmails
              ...
              .map(({ email, selected }) => (
                ...
                >
                  ...
                  <i
                    className={clsx('size-5', selected ? 'icon-check-filled' : 'icon-circle-outline text-quaternary')}
                  />
                </div>
              ))}
          </div>
