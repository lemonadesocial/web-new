...
export function AddInvitesModal({ emails, event, onBack }: { emails: string[]; event: Event; onBack: () => void }) {
  const [inviteEvent, { loading }] = useMutation(InviteEventDocument);
  const [tab, setTab] = useState<'invites' | 'guests'>('invites');
  ...
  if (tab === 'guests') return <AddGuestsModal emails={emails} event={event} onBack={() => setTab('invites')} />;
    ...
    <div className="w-[448px]">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-tertiary">Guests will be automatically approved when they complete their registration.</p>
            </div>
          </div>
        </div>
        ...
        <p className="text-sm text-tertiary">
          ...
          <span className="text-accent-500 cursor-pointer" onClick={() => setTab('guests')}>Add Guests Directly</span>
        </p>
      </div>
