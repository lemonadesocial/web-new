  ...
  return (
    <Card.Root className="w-[360px]">
      ...
      <Card.Content className="flex flex-col gap-4 max-w-[448px]">
        ...
        {events.length === 1 && (
          <Card.Root className="overflow-visible" onClick={() => handleOpen(events[0].shortid)}>
            <Card.Content className="flex gap-3 px-3 py-2 items-center">
              {!!events[0].new_new_photos?.length ? (
                <div
                  style={{
                    background: `url(${generateUrl(events[0].new_new_photos[0], { resize: { width: 36, height: 36, fit: 'cover' } })})`,
                  }}
              ...
              <div className="flex-1">
                <p className="line-clamp-1">{events[0].title}</p>
                <p className="text-xs text-tertiary">
                  {formatWithTimezone(events[0].start, 'EEE, MMM dd, hh:mm a OOOO', events[0].timezone)}
                </p>
              </div>
          ...
          <div className="flex flex-col gap-1.5 max-h-[300px] overflow-auto no-scrollbar">
            <Card.Root className="overflow-visible">
              <Card.Content className="flex gap-3 px-3 py-2 items-center">
                ...
                <div className="flex-1">
                  <p className="line-clamp-1">{events[0].title}</p>
                  <p className="text-xs text-tertiary">{getEventLocation(events[0])}</p>
                </div>
              </Card.Content>
            </Card.Root>
