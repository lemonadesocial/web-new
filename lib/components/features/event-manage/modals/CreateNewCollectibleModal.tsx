...
export function CreateNewCollectibleModal({ event }: CreateNewCollectibleModalProps) {
  ...
  const [createPoapDrop] = useMutation(CreatePoapDropDocument);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const { data: responseData, error } = await createPoapDrop({
        variables: {
          input: {
      ...
      if (responseData?.createPoapDrop) {
        toast.success('Collectible created successfully!');
        modal.close();
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to create collectible');
    } finally {
      setIsSaving(false);
    }
  };
