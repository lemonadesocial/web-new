...
import { EthereumAccount, NewPaymentAccount, NewPaymentProvider, PaymentAccountType } from "$lib/graphql/generated/backend/graphql";
      ...
      <div className="rounded-md border border-card-border bg-card overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-(--color-divider)">
            ...
            {cryptoAccounts?.map((vault) => (
              <VaultRow key={vault?._id} vault={vault!} />
            ))}
          </tbody>
        </table>
      </div>
...
function VaultRow({ vault }: { vault: NewPaymentAccount }) {
  const handleEdit = () => {
  ...
  return (
    <tr>
      <td className="px-4 py-3">
        <p>{vault.provider === NewPaymentProvider.Stripe ? 'Stripe' : vault.title || 'Untitled Vault'}</p>
      </td>
      <td className="px-4 py-3">
        <Chip className="rounded-full" variant="secondary" size="xxs">
...
function PaymentNetwork({ vault }: { vault: NewPaymentAccount }) {
