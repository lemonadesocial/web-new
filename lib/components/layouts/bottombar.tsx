...
export function BottomBar() {
  ...
  const menu = useMemo(() => {
    const menu: Array<{ icon: string | React.ReactElement; path: string }> = [
      { icon: 'icon-home', path: '/' },
    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 w-full p-4 flex justify-center">
      <div className="flex items-center gap-2 p-2 rounded-full border bg-overlay-secondary [backdrop-filter:var(--backdrop-filter)]">
        {menu.map((item) => (
          <div
            key={item.path}
            className={clsx('h-[44px] min-w-[44px] flex items-center justify-center rounded-full cursor-pointer', isActive(item) && 'bg-[var(--btn-secondary)]')}
