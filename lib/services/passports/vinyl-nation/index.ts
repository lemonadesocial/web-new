...
const boldFontPath = path.join(process.cwd(), 'data', 'vinyl-nation-passport', 'semibold.ttf');
const regularFontFamily = 'vinyl-nation-passport-font-regular';
const boldFontFamily = 'vinyl-nation-passport-font-semibold';
...
const firstLineY = 185;
const secondLineY = 476;

const avatarOffset: Point = { x: 64, y: 78 };
const usernameOffset: Point = { x: 552, y: 276 };

const passportIdOffset: Point = { x: 376, y: 594 };
...
const getPassportIdImageBuffer = async (passportId: string) => {
  ...
  return getTextImageBuffer(outputWidth, outputHeight, regularFont, passportId, passportIdOffset, '#DCD9C1');
};
